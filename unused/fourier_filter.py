...
def filter(data, col, fft, frequency):
    ...
    low = highest_amplitude_freq - 1
    high = highest_amplitude_freq + 1
    if low <= 0.1:
        low = 0.1
    # sos = scipy.signal.butter(2, [low, high], 'bandpass', fs=SAMPLE_RATE, output='sos', analog=False)
    sos = scipy.signal.butter(2, 7.5, 'lowpass', fs=SAMPLE_RATE, output='sos', analog=False)
    minus_mean = data[col] - np.mean(data[col])
    return np.abs(scipy.signal.sosfilt(sos, minus_mean))
